# Ls_Hospitality — Hyvä theme support

This module ships Hyvä-themed rendering for hospitality **customizable options**
on the product page. Under Luma nothing here applies — the `hyva_*` layout
handle and the `hyva_config_generate_before` observer only activate when a Hyvä
theme is installed.

## What it does on Hyvä

`view/frontend/layout/catalog_product_view.xml` re-points the customizable-option
blocks to Luma-era LS templates (swatch / `options-modal` markup) that are not
compatible with Hyvä. On Hyvä, `view/frontend/layout/hyva_catalog_product_view.xml`
restores the stock `Magento_Catalog` option templates, so the options render like
the rest of the Hyvä UI (styled selects / button-pill radios & checkboxes).
...
If the customizable-option controls render with mismatched widths, the Tailwind
build has not been run (or `hyva:config:generate` was not run before it).
